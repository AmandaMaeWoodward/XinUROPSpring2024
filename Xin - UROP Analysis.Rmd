...
clean_data$mr2 <- (clean_data$metacognition2ReflectQ4 + clean_data$metacognition1ReflectQ5)  
clean_data$mr3 <- (clean_data$metacognition3ReflectQ4 + clean_data$metacognition1ReflectQ5)  
clean_data$pre_mr <- (clean_data$problemsolvingReflectionHelpful + clean_data$problemsolvingReflectionunHelpful + clean_data$problemSolvingReflectionStepbyStep)
```
###Total score of Metacognitive reflection
```{r}
clean_data$TotalMRScore <- clean_data$mr1 + clean_data$mr2 + clean_data$mr3
```

###TotalMR score (including pre_mr) 
```{r}
clean_data$ALLMRScore <- clean_data$pre_mr+ clean_data$mr1 + clean_data$mr2 + clean_data$mr3
```
###total confidence in R and Stat of each case
```{r}
clean_data$AverageRConfidence<- (clean_data$metacognition1ConfidenceR+ clean_data$metacognition2ConfidenceR +clean_data$metacognition3ConfidenceR)/3
clean_data$AverageStatisticsConfidence<- (clean_data$metacognition1ConfidenceStatistics + clean_data$metacognition2ConfidenceStatistics +clean_data$metacognition3ConfidenceStatistics)/3
```

#Histogram
Amanda Note: make sure to update these with the columns from the dataset
```{r}
...
hist(TotalMRScore, main = "metacognitive reflection in three case study")
